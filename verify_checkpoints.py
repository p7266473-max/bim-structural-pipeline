...
def verify_geometry_equilibrium(geojson_data, opensees_results):
    ...
    """
    # Load geometry equilibrium rules
    ...
    total_reaction = 0.0
    for nid, ndata in opensees_results.get("nodes", {}).items():
        # reactions: [Rx, Ry, Mz]
        total_reaction += ndata["reactions"][1] # y-direction reaction
...
def verify_tributary_loads(geojson_data, opensees_results):
    """
    ...
    # For a 2D line layout, tributary length for columns/supports is half the distance of adjacent elements.
    support_nodes = []
    node_coords = {}
    ...
    # Estimate load distribution comparison
    tributary_results = []
    for nid in support_nodes:
        coord = node_coords[nid]
        pt = Point(coord)
        # Find connecting beam lengths
        trib_length = 0.0
        for beam in beams:
            if beam.distance(pt) < 0.05:
                trib_length += beam.length / 2.0 # Tributary length contribution
                
        tributary_load = trib_length * (dead_load + live_load)
        actual_reaction = opensees_results.get("nodes", {}).get(nid, {}).get("reactions", [0, 0, 0])[1]
...
def verify_material_detailing(opensees_results):
    """
    ...
    for feat in rules.get("features", []):
        ...
        if props.get("rule_type") == "concrete_fy":
            ...
            f_yk = float(props.get("value", 500.0))
    ...
    for elem in opensees_results.get("elements", []):
        # Peak moment is typically element forces index 2 (M1) or index 5 (M2)
        forces = elem.get("forces", [0.0]*6)
        M_max = max(abs(forces[2]), abs(forces[5])) # Bending moment in kNm
        
        # Concrete capacity calculation: simplified plastic moment capacity
        # M_rd = 0.138 * f_ck * b * d^2
        w_mm = float(elem.get("properties", {}).get("section_w_mm", 300))
        h_mm = float(elem.get("properties", {}).get("section_h_mm", 600))
        d_mm = h_mm - 50.0 # d = h - cover (50mm default)
        
        M_capacity = 0.138 * f_ck * w_mm * (d_mm ** 2) / 1e6 # in kNm
        dc_ratio = M_max / M_capacity if M_capacity > 0 else 0.0
        
        # Detailing: select rebar count to satisfy bending moment demand
        # A_s = M_max * 1e6 / (0.87 * f_yk * z) where z = 0.95 * d
        z = 0.95 * d_mm
        A_s_required = (M_max * 1e6) / (0.87 * f_yk * z) if M_max > 0 else 0.0
        
        # Map to rebar choices
        chosen_bar = 20
        for r_dia in rebar_choices:
            if r_dia >= 12: # Prefer standard longitudinal bars
                chosen_bar = r_dia
                bar_area = 3.14159 * (chosen_bar / 2.0) ** 2
                qty = int(np.ceil(A_s_required / bar_area)) if A_s_required > 0 else 2
                qty = max(qty, 2) # minimum 2 bars
                if qty <= 6: # Standard layout density check
                    break
